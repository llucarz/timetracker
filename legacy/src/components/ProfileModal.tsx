...
export function ProfileModal({ isOpen, onClose, defaultSchedule }: ProfileModalProps) {
  const [mode, setMode] = useState<"same" | "different">("same");
  const [weeklyTarget, setWeeklyTarget] = useState("35");
  const [workdaysPerWeek, setWorkdaysPerWeek] = useState("5");

  // Same schedule mode
  const [arrival, setArrival] = useState(defaultSchedule?.arrival || "09:00");
  const [pauseStart, setPauseStart] = useState(defaultSchedule?.pauseStart || "12:30");
  const [pauseEnd, setPauseEnd] = useState(defaultSchedule?.pauseEnd || "13:30");
  const [departure, setDeparture] = useState(defaultSchedule?.departure || "18:00");
